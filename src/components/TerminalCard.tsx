    ...
    >
      ...
      <div className="flex justify-end items-center mb-8 max-sm:flex-col max-sm:gap-3 max-[480px]:flex-col-reverse max-[480px]:gap-3 max-[480px]:mb-6">
        <div className="flex gap-2 ml-auto max-sm:w-full max-sm:justify-center max-sm:flex-wrap max-sm:ml-0">
          <IconButton href="https://leetcode.com/u/SlightlyAcidic/" icon="fa-solid fa-code" ariaLabel="LeetCode" />
        </div>
      </div>

      {/* Main Content */}
      <div className="flex gap-6 items-start mb-8 max-sm:flex-col max-sm:items-center max-sm:text-center">
        {/* Profile Image */}
        <div
          className="flex-shrink-0 cursor-pointer relative inline-block"
          onClick={onProfileClick}
        >
            <div className="absolute -top-7 left-1/2 -translate-x-1/2 font-sans text-[11px] tracking-[0.2px] lowercase py-[3px] px-[9px] rounded-full flex items-center gap-1 whitespace-nowrap game-hint-label shadow-sm"
              style={{ background: 'var(--accent-color)', color: 'var(--button-text)' }}>
            <svg
        ...
        <div className="flex-1 flex flex-col gap-2 pt-1">
          <div className="flex items-center gap-4 max-sm:flex-col max-sm:gap-2">
            <h1 className="text-[32px] font-bold text-[var(--text-color)] m-0 leading-none tracking-[-0.02em] inline-flex items-center gap-2
              max-md:text-[32px]
              max-sm:text-[28px]
              max-[480px]:text-2xl
              max-[360px]:text-[22px]"
          ...
          <div className="flex items-center gap-2 text-sm text-[var(--text-color)] mt-1 opacity-90 max-sm:justify-center max-sm:text-[13px] max-[480px]:text-xs">
            <i className="fa-solid fa-envelope text-[13px]" style={{ color: 'var(--accent-color)' }} />
            <span>chiragj831@gmail.com</span>
          </div>
        </div>
      </div>
      ...
      <div className="text-center text-sm leading-relaxed text-[var(--text-color)] opacity-70 max-w-[90%] mx-auto font-normal max-sm:text-[13px] max-sm:max-w-full max-[480px]:text-xs">
        I make things that work. Sometimes they matter. Now exploring Kubernetes.
      </div>
    </div>
  );
}
